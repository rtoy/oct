...
(defun sqrt-qd (a)
  ...
  (let* ((r (make-qd-d (/ (sqrt (the (double-float (0d0))
				  (qd-0 a)))) 0d0 0d0 0d0))
	 (h (mul-qd a (make-qd-dd .5w0 0w0)))
	 (half (make-qd-d .5d0 0d0 0d0 0d0)))
...
(defun read-qd (stream)
  (labels ((read-digits (s)
	     ;; Read a sequence of digits and return the decimal value and the character that terminated the sequence.
	     (let ((val 0)
		   (ch nil)
		   (count 0))
	       (loop
